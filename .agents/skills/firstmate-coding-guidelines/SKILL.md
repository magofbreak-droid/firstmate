...
- Run `bin/fm-verify.sh` before treating a shared tracked change as done; it invokes `bin/fm-lint.sh`, the single owner of the lint definition (file set, config, pinned shellcheck version, and pinned actionlint workflow lint), and is the identical local and GitHub Actions gate.
