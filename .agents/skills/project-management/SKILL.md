...
- `direct-PR` runs the canonical verifier, opens a PR, and requires terminal checks for the recorded exact head before merge.
...
Legacy `no-mistakes` and `no-mistakes-prod-only` annotations remain readable as inactive migration compatibility and resolve to `direct-PR` with a warning.
