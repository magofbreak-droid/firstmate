...
`bin/fm-pr-ci.sh` snapshots the union of classic branch-protection checks and effective repository or organization ruleset checks around exact-SHA results, preserves every provider constraint, and refuses when that requirement evidence is missing, incomplete, ambiguous, changed, or not terminal-successful.
When both GitHub requirement APIs return the exact private-plan feature refusal, an explicitly supplied repository-and-base-bound policy may provide the requirement set; [`configuration.md`](configuration.md#private-github-required-check-policy) owns its schema and the immutable task-binding contract.
