...
They source `bin/fm-gate-refuse-lib.sh` and stay silent for an inactive legacy migration gate agent identified by `NO_MISTAKES_GATE` or a `.no-mistakes/repos/*.git` git-common-dir.
