...
run_spawn_symlink_case() {  # <label> <physical|logical>
  ...
  out=$(run_spawn_case "$ROOT" "$fb" "$log" "$state" "$data" "$config" "$proj" -- "$id" "$proj" claude --mode direct-PR --yolo off 2>&1)
  rc=$?
...
test_spawn_refuses_unknown_backend_flag() {
  ...
  out=$(FM_ROOT_OVERRIDE='' FM_HOME='' FM_STATE_OVERRIDE='' FM_DATA_OVERRIDE='' \
    ...
    "$ROOT/bin/fm-spawn.sh" nope-backend-z1 projects/none claude --mode direct-PR --yolo off --backend bogus 2>&1)
  status=$?
...
test_spawn_refuses_codex_app_backend_flag() {
  ...
  out=$(FM_ROOT_OVERRIDE='' FM_HOME='' FM_STATE_OVERRIDE='' FM_DATA_OVERRIDE='' \
    FM_PROJECTS_OVERRIDE='' FM_CONFIG_OVERRIDE='' FM_SPAWN_NO_GUARD=1 \
    "$ROOT/bin/fm-spawn.sh" nope-codex-app-z1 projects/none claude --mode direct-PR --yolo off --backend codex-app 2>&1)
  status=$?
...
test_spawn_refuses_unknown_fm_backend_env() {
  ...
  out=$(FM_ROOT_OVERRIDE='' FM_HOME='' FM_STATE_OVERRIDE='' FM_DATA_OVERRIDE='' \
    ...
    "$ROOT/bin/fm-spawn.sh" nope-backend-z2 projects/none claude --mode direct-PR --yolo off 2>&1)
  status=$?
...
test_spawn_default_backend_writes_no_meta_field() {
  ...
  out=$(PATH="$fb:$PATH" FM_ROOT_OVERRIDE="$ROOT" \
    ...
    "$ROOT/bin/fm-spawn.sh" "$id" "$proj" claude --mode direct-PR --yolo off --backend tmux 2>&1)
