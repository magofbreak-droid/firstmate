...
#   - direct-PR project clones require no legacy no-mistakes initialization and
#     the parent project clone is never mutated (no write through a project)
...
EOF
  ...
  # One combined fakebin: tmux + treehouse (spawn/send/teardown) and a legacy
  # no-mistakes sentinel that must remain unused during direct-PR seeding.
...
phase_seed() {
  ...
  # Direct-PR seeding never invokes or writes the retired no-mistakes initializer.
  assert_absent "$SUB/projects/gamma/.no-mistakes-init" "seed invoked retired no-mistakes init in the subhome"
  assert_absent "$SUB/projects/gamma/.no-mistakes-doctor" "seed invoked retired no-mistakes doctor in the subhome"
  assert_absent "$HOME_DIR/projects/gamma/.no-mistakes-init" "seed wrote retired no-mistakes state through the parent project"
  ...
  pass "seed: registry scope+projects, charter copied, clones+origins, no legacy initialization"
}
